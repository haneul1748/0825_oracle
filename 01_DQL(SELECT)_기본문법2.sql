...
SELECT * FROM EMPLOYEE; -- 이 방법은 사용하면 안됨
-- * 사용 시 성능에 영향을 끼침
...
-- 소문자로 작성을해도 잘 작동을 하지만 대문자로 작성하는 습관을 들이는것이 좋음
...
SELECT
 ...
 WHERE
 	   ENT_YN = 'N';
-- 4. EMPOLOYEE테이블에서 연봉이 5000이상인 사원들의 이름, 연봉 조회
...
-- EMPLOYEE테이블로부터 급여컬럼이 값이 350만원 이상이고 500만원 이하인 사원들의
-- 사번(EMP_ID), 이름, 급여를 조회
SELECT
	   EMP_ID
	 , EMP_NAME
...
-- EMPLOYEE테이블에서 급여가 350만원이거나 500을 초과하는 사원들의 사번, 이름, 직급코드
SELECT 
	   EMP_ID
	 , EMP_NAME
	 , JOB_CODE
 ...
 *  비교대상컬럼 LIKE 'A_' => 컬럼값에 'A'앞에 무조건 2글자가 있어야만 패턴에 만족 
 */
...
SELECT
  ...
  FROM
  	   EMPLOYEE
 WHERE 
 	 --BONUS IS NOT NULL; -- EMPLOYEE테이블로부터 보너스를 받는 사원들의 사원명, 보너스 조회
  	   BONUS IS NULL;
 ...
 * <☆★☆★☆★☆★☆★☆★☆★☆ ORDER BY 절 ☆★☆★☆★☆★☆★☆★☆★ >
 * 정렬용도로 사용하는 구문
 * SELECT문에 가장 마지막에 작성하는 문법 + 실행 순서또한 가장 마지막
