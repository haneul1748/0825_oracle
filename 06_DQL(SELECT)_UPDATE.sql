...
*/
/*
 * INSERT : 테이블에 새로운 행을 추가해주는 구문
 * [ 표현법 ]
 * 1) INSERT INTO 테이블명 VALUES(값, 값, 값, ...);
 * => 해당 테이블에 모든 컬럼에 추가하고자 하는 값을 직접 입력해서 한 행을 INSERT할 떄사용
 ...
 * INSERT는 무조건 한 행 단위로 추가가되기 때문에 작성하지 않는 컬럼은 NULL값이 들어감
 * 
 * 주의할 점 ) NOT NULL이 달려있는 컬럼은 반드시 테이블 명 뒤에 컬럼명을 적어야함
 * 예외 사항 ) NOT NULL 제약조건이 달려있는데 기본값이 있는 경우는 DEFAULT 값이 들어간다
 * 
 */
 ...
 * => VALUES 대신에 서브쿼리르 작성해서INSERT 할 ㅛㅜ 있음
 */
--새테이블 하나 만들시
...
--EMPLOYEE테이블에 존재하는 모든 사원 사원명과 부서명을 새로만든EMP01에 INSERT
INSERT
  INTO
       EMP_01
       (
...
--DEPT_COPY테이블에서 총무부의 부서명을 미래사업부로 변경
...
-- 전체 행의 모든 DEPT_TITLE 컬럼 값이 전부 미래 사업부로 UPDATE
